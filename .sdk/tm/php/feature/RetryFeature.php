...
class AnipubRetryFeature extends AnipubBaseFeature
{
    ...
    public function init(AnipubContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AnipubContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AnipubContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AnipubContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
