...
class FtCtrl extends AnipubControl
{
...
class FtClient
{
    ...
    private AnipubUtility $utility;

    public function __construct(AnipubUtility $utility, array $options)
    {
    ...
    public function get_utility(): AnipubUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AnipubContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AnipubContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AnipubConfig::make_feature($name);
    ...
    private static function buildUrl(AnipubSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AnipubError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AnipubResponse($fetched);
        }
    ...
    private function fail(AnipubContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AnipubContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
