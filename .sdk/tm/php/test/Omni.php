...
//    only - a typed AnipubContext there would make every ctx
...
final class AnipubOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new AnipubContext($clean, null);
    ...
    public static function err_from_map($m): ?AnipubError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AnipubError(is_string($code) ? $code : '', $msg);
    }
