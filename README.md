...
AniPub API client, generated from the OpenAPI spec.

AniPub API provides comprehensive metadata on anime, including streaming links, character details, and MAL integration.

Learn more about AniPub API at [anipub.xyz](https://anipub.xyz).

This is an unofficial SDK for the AniPub public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
```bash
cd go-mcp && go build -o anipub-mcp .
```
...
```json
{
  "mcpServers": {
    "anipub": {
      "command": "/abs/path/to/anipub-mcp"
    }
  }
}
```
...
```go
import sdk "github.com/voxgig-sdk/anipub-sdk/go"
...
Generated from the AniPub API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
