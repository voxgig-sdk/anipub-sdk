...
boru-driven command-line client **and** interactive REPL for the Anipub
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/anipub-cli)
make build
...
./anipub-cli load 1 anime            # {id:1} shorthand
./anipub-cli load '{id:1}' anime       # explicit match map

# 5. Override the API base URL for a single call
ANIPUB_BASE=https://api.example.com ./anipub-cli load 1 anime
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/anipub-cli
   ```
...
```sh
./anipub-cli load 1 anime          # scalar shorthand for {id:1}
./anipub-cli load '{id:1}' anime     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `anipub>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/anipub-cli`. |
...
(`./anipub-cli <expr>`) and interactively (the REPL), and expressions compose the
