// Command Anipub-mcp is an MCP server exposing the Anipub
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Anipub_list  — list records of an entity
//	Anipub_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: ANIPUB_APIKEY carries the API key and
	// ANIPUB_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ANIPUB_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ANIPUB_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAnipubSDK(opts)
