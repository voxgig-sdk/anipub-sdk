...
func TestAnimeDirect(t *testing.T) {
	t.Run("direct-load-anime", func(t *testing.T) {
		setup := animeDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-anime", _mode); _shouldSkip {
			if _reason == "" {
...
func animeDirectSetup(mockres any) *animeDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"ANIPUB_TEST_ANIME_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ANIPUB_TEST_ANIME_ENTID"]; ok {
