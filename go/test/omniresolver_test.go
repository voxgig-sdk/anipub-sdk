...
import (
	...
	omni "github.com/voxgig-sdk/anipub-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AnipubError); is {
				out := map[string]any{
					"name":    "AnipubError",
...
func omniCtx(arg any, client *sdk.AnipubSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AnipubSDK); is {
			client = live
