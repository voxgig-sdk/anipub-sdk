...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("ANIPUB_TEST_LIVE") == "TRUE" ||
		os.Getenv("ANIPUB_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("ANIPUB_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.AnipubSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.AnipubError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.AnipubSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.AnipubError{Msg: msg, Code: code}
}
