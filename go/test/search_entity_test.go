...
func TestSearchEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ANIPUB_TEST_SEARCH_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		searchRef01DataDt0LoadResult := core.ToMapAny(searchRef01DataDt0Loaded)
...
func searchBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ANIPUB_TEST_SEARCH_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ANIPUB_TEST_SEARCH_ENTID"])
