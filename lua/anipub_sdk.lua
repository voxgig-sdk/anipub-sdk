...
AnipubSDK._make_feature = _make_feature


function AnipubSDK.new(options)
  local self = setmetatable({}, AnipubSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function AnipubSDK:options_map()
...
function AnipubSDK:get_utility()
  return Utility.copy(self._utility)
end


function AnipubSDK:get_root_ctx()
  return self._rootctx
end


function AnipubSDK:prepare(fetchargs)
...
function AnipubSDK:direct(fetchargs)
...
-- by passing it.
function AnipubSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function AnipubSDK:graphql(query, variables, ctrl)
...
-- Idiomatic facade: client:Find():list() / client:Find():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function AnipubSDK:Find(data)
  local EntityMod = require("entity.find_entity")
  if data == nil then
    if self._find == nil then
      self._find = EntityMod.new(self, nil)
    end
    return self._find
  end
  return EntityMod.new(self, data)
...
function AnipubSDK:FullAnimeDetail(data)
  local EntityMod = require("entity.full_anime_detail_entity")
  if data == nil then
    if self._full_anime_detail == nil then
      self._full_anime_detail = EntityMod.new(self, nil)
    end
    return self._full_anime_detail
  end
  return EntityMod.new(self, data)
end
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function AnipubSDK:Search(data)
...
-- Idiomatic facade: client:StreamingDetail():list() / client:StreamingDetail():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function AnipubSDK:StreamingDetail(data)
  local EntityMod = require("entity.streaming_detail_entity")
  if data == nil then
    if self._streaming_detail == nil then
      self._streaming_detail = EntityMod.new(self, nil)
    end
    return self._streaming_detail
  end
  return EntityMod.new(self, data)
end
...
function AnipubSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = AnipubSDK.new(sdkopts)
  sdk.mode = "test"
