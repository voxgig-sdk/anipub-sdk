...
local FindEntity = {}
FindEntity.__index = FindEntity


function FindEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FindEntity)
  self._name = "find"
  self._client = client
...
function FindEntity:get_name()
  return self._name
end


function FindEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FindEntity.new(self._client, opts)
end
...
function FindEntity:mark_deleted()
  self._deleted = true
end


function FindEntity:deleted()
  return true == self._deleted
end


function FindEntity:data_set(args)
  if args ~= nil then
...
function FindEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FindEntity:match_set(args)
  if args ~= nil then
...
function FindEntity:stream(action, args, callopts)
...
---@param reqmatch FindLoadMatch
---@param ctrl? table
---@return Find
---@return string? err
function FindEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function FindEntity:_run_op(ctx, post_done)
  local utility = self._utility
