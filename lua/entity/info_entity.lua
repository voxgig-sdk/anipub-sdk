...
local InfoEntity = {}
InfoEntity.__index = InfoEntity


function InfoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InfoEntity)
  self._name = "info"
  self._client = client
...
function InfoEntity:get_name()
  return self._name
end


function InfoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InfoEntity.new(self._client, opts)
end
...
function InfoEntity:mark_deleted()
  self._deleted = true
end


function InfoEntity:deleted()
  return true == self._deleted
end


function InfoEntity:data_set(args)
  if args ~= nil then
...
function InfoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InfoEntity:match_set(args)
  if args ~= nil then
...
function InfoEntity:stream(action, args, callopts)
...
---@param reqmatch InfoLoadMatch
---@param ctrl? table
---@return Info
---@return string? err
function InfoEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function InfoEntity:_run_op(ctx, post_done)
  local utility = self._utility
