...
local StreamingDetailEntity = {}
StreamingDetailEntity.__index = StreamingDetailEntity


function StreamingDetailEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StreamingDetailEntity)
  self._name = "streaming_detail"
  self._client = client
...
function StreamingDetailEntity:get_name()
  return self._name
end


function StreamingDetailEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StreamingDetailEntity.new(self._client, opts)
end
...
function StreamingDetailEntity:mark_deleted()
  self._deleted = true
end


function StreamingDetailEntity:deleted()
  return true == self._deleted
end


function StreamingDetailEntity:data_set(args)
  if args ~= nil then
...
function StreamingDetailEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StreamingDetailEntity:match_set(args)
  if args ~= nil then
...
function StreamingDetailEntity:match_get()
...
function StreamingDetailEntity:stream(action, args, callopts)
...
---@param reqmatch StreamingDetailLoadMatch
---@param ctrl? table
---@return StreamingDetail
---@return string? err
function StreamingDetailEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function StreamingDetailEntity:_run_op(ctx, post_done)
  local utility = self._utility
