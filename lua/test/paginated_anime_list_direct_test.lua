...
describe("PaginatedAnimeListDirect", function()
  it("should direct-list-paginated_anime_list", function()
    local setup = paginated_anime_list_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-paginated_anime_list", setup.live and "live" or "unit")
  ...
  it("should direct-load-paginated_anime_list", function()
    local setup = paginated_anime_list_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-paginated_anime_list", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
