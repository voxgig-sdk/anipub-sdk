...
class AnipubSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = AnipubHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = AnipubHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!AnipubFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, AnipubFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return AnipubUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = AnipubHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = AnipubHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = AnipubHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new AnipubError($op . "_allow",
                "AnipubSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = AnipubHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new AnipubError("graphql_error",
                "AnipubSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Find()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->find()
    // resolves here too.
    public function Find($data = null)
    {
        require_once __DIR__ . '/entity/find_entity.php';
        if ($data === null) {
            if ($this->_find === null) {
                $this->_find = new FindEntity($this, null);
            }
            return $this->_find;
        }
        return new FindEntity($this, $data);
    }
    ...
    // PHP method names are case-insensitive, so lowercase $client->full_anime_detail()
    // resolves here too.
    public function FullAnimeDetail($data = null)
    {
        require_once __DIR__ . '/entity/full_anime_detail_entity.php';
        if ($data === null) {
            if ($this->_full_anime_detail === null) {
                $this->_full_anime_detail = new FullAnimeDetailEntity($this, null);
            }
            return $this->_full_anime_detail;
        }
        return new FullAnimeDetailEntity($this, $data);
    }
    ...
    private $_streaming_detail = null;

    // Canonical facade: $client->StreamingDetail()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->streaming_detail()
    // resolves here too.
    public function StreamingDetail($data = null)
    {
        require_once __DIR__ . '/entity/streaming_detail_entity.php';
        if ($data === null) {
            if ($this->_streaming_detail === null) {
                $this->_streaming_detail = new StreamingDetailEntity($this, null);
            }
            return $this->_streaming_detail;
        }
        return new StreamingDetailEntity($this, $data);
    }
