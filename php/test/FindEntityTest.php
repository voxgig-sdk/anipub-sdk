...
class FindEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Find(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = find_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "find." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ANIPUB_TEST_FIND_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $find_ref01_data = null;
        if (count($find_ref01_data_raw) > 0) {
            $find_ref01_data = Helpers::to_map($find_ref01_data_raw[0][1]);
        }

        // LOAD
        $find_ref01_ent = $client->Find(null);
...
function find_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/find/FindTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["find01", "find02", "find03"] as $k) {
    ...
    $entid_env_raw = getenv("ANIPUB_TEST_FIND_ENTID");
