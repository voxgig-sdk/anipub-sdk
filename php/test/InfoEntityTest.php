...
class InfoEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Info(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = info_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "info." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ANIPUB_TEST_INFO_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $info_ref01_data = null;
        if (count($info_ref01_data_raw) > 0) {
...
function info_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/info/InfoTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["info01", "info02", "info03"] as $k) {
    ...
    $entid_env_raw = getenv("ANIPUB_TEST_INFO_ENTID");
