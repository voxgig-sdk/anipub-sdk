...
// Anipub SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class AnipubMakeError
{
    public static function call(?AnipubContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new AnipubContext([], null);
        }
        $op = $ctx->op ?? new AnipubOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new AnipubResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof AnipubError) ? $err->msg : (string)$err;
        $msg = "AnipubSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new AnipubError('', $msg, $ctx);
