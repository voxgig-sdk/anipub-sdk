...
// Anipub SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class AnipubMakePoint
{
    public static function call(AnipubContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof AnipubError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = AnipubHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = AnipubHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
