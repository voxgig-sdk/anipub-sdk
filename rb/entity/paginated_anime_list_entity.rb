...
class PaginatedAnimeListEntity
  ...
  def make
    opts = @_entopts.dup
    PaginatedAnimeListEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AnipubHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PaginatedAnimeList, Hash] the current PaginatedAnimeList data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AnipubHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PaginatedAnimeList fields)
  def match_get
  ...
  #
  # @param reqmatch [PaginatedAnimeListLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.PaginatedAnimeList.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [PaginatedAnimeList, Hash] the loaded PaginatedAnimeList; raises AnipubError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AnipubHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [PaginatedAnimeListListMatch, Hash, nil] match filter (any subset of
  #   PaginatedAnimeList fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<PaginatedAnimeList>, Array] the matching PaginatedAnimeList items; raises AnipubError on failure
