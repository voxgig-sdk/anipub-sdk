...
class StreamingDetailEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = streaming_detail_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "streaming_detail." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ANIPUB_TEST_STREAMING_DETAIL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    streaming_detail_ref01_data = nil
    if streaming_detail_ref01_data_raw.length > 0
...
def streaming_detail_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "streaming_detail", "StreamingDetailTestData.json")
  ...
  entid_env_raw = ENV["ANIPUB_TEST_STREAMING_DETAIL_ENTID"]
