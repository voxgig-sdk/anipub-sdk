...
(0, node_test_1.describe)('AnimeDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-anime', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-anime', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['ANIPUB_TEST_ANIME_ENTID'];
