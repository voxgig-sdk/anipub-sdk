...
(0, node_test_1.describe)('PaginatedAnimeListDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-paginated_anime_list', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-paginated_anime_list', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-paginated_anime_list', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-paginated_anime_list', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.ANIPUB_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.AnipubSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['ANIPUB_TEST_PAGINATED_ANIME_LIST_ENTID'];
    ...
    const client = new __1.AnipubSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=PaginatedAnimeListDirect.test.js.map