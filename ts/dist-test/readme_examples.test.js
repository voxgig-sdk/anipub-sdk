...
function isRunnable(code) {
    return (/new\s+AnipubSDK\b/.test(code) ||
        /\bAnipubSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+AnipubSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/AnipubSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
