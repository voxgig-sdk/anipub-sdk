...
describe('FullAnimeDetailDirect', async () => {
  ...
  test('direct-load-full_anime_detail', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-full_anime_detail', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ANIPUB_TEST_FULL_ANIME_DETAIL_ENTID']
