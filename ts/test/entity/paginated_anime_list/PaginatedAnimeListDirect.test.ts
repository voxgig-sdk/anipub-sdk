...
describe('PaginatedAnimeListDirect', async () => {
  ...
  test('direct-load-paginated_anime_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-paginated_anime_list', setup.live)) return
  ...
  test('direct-list-paginated_anime_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-paginated_anime_list', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ANIPUB_TEST_PAGINATED_ANIME_LIST_ENTID']
