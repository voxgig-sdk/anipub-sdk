...
describe('PaginatedAnimeListEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'paginated_anime_list.' + op, live)) return
    }
    ...
    let paginated_anime_list_ref01_data = Object.values(setup.data.existing.paginated_anime_list)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/paginated_anime_list/PaginatedAnimeListTestData.json')
  ...
  const idmapEnvVal = process.env['ANIPUB_TEST_PAGINATED_ANIME_LIST_ENTID']
