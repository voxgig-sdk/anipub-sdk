...
describe('StreamingDetailDirect', async () => {
  ...
  test('direct-load-streaming_detail', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-streaming_detail', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ANIPUB_TEST_STREAMING_DETAIL_ENTID']
